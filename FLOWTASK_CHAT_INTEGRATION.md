...
> **FlowTask API URL:** `https://proofhub-trello-replica.onrender.com`
...
11. [Chat Application API Reference](#11-chat-application-api-reference)  
...
### 9.1 Webhook Event Handler Coverage

All 15 domain events are now handled by the Chat Application backend:

| Event | Handler File | Status |
|---|---|---|
| `PROJECT_CREATED` | `projectEventHandler.js` | ✅ Implemented + botNotifier routing |
| `PROJECT_UPDATED` | `projectEventHandler.js` | ✅ Implemented |
| `PROJECT_DELETED` | `projectEventHandler.js` | ✅ Implemented |
| `PROJECT_MEMBER_ASSIGNED` | `projectEventHandler.js` | ✅ Implemented (new) |
| `PROJECT_MEMBER_REMOVED` | `projectEventHandler.js` | ✅ Implemented |
| `TASK_CREATED` | `taskEventHandler.js` | ✅ Implemented |
| `TASK_UPDATED` | `taskEventHandler.js` | ✅ Implemented |
| `TASK_DELETED` | `taskEventHandler.js` | ✅ Implemented |
| `TASK_STATUS_CHANGED` | `taskEventHandler.js` | ✅ Implemented (new) |
| `TASK_DUE_DATE_CHANGED` | `taskEventHandler.js` | ✅ Implemented (new) |
| `TASK_COMMENT_ADDED` | `taskEventHandler.js` | ✅ Implemented (new) |
| `TIME_ENTRY_ADDED` | `taskEventHandler.js` | ✅ Implemented (new) |
| `USER_REGISTERED` | `userEventHandler.js` | ✅ Implemented (new) |
| `USER_VERIFIED` | `userEventHandler.js` | ✅ Implemented (new) |
| `ANNOUNCEMENT_CREATED` | `announcementEventHandler.js` | ✅ Implemented + botNotifier routing |

### 9.2 Bot Notification Routing

The `botNotifier` service (`server/services/botNotifier.js`) routes notifications to system channels:

| Method | Target Channels | Triggered By |
|---|---|---|
| `notifyAdmins(content)` | `#flowtask-admin` | Project creation, user events |
| `notifyManagers(content)` | `#flowtask-managers` | Project creation, user verification |
| `notifyAnnouncements(content)` | `#flowtask-announcements` | Announcement creation |
| `onProjectCreated(data)` | `#flowtask-admin` + `#flowtask-managers` | PROJECT_CREATED handler |
| `onUserVerified(data)` | `#flowtask-admin` + dept `#flowtask-managers` | USER_VERIFIED handler |
| `onAnnouncementCreated(data)` | `#flowtask-admin` | ANNOUNCEMENT_CREATED handler |

### 9.3 Security Enhancements

| Feature | Implementation | File |
|---|---|---|
| CORS origin fix | `CORS_ORIGINS` env variable properly referenced | `server/config/environment.js` |
| Message access middleware | `requireMessageAccess()` — resolves message → channel → checks membership | `server/modules/auth/auth.middleware.js` |
| Socket channel:join auth | Membership verification before joining channel rooms | `server/sockets/socketManager.js` |
| Search scope filtering | Message search filtered by user's accessible channels | `server/modules/messages/message.service.js` |
| Request validation | Zod schemas for all message, channel, and thread endpoints | `server/middleware/validate.js`, `server/middleware/schemas.js` |

### 9.4 Manager Auto-Join Logic

When a user's role is updated via `USER_UPDATED`:

- **Promoted to admin** → auto-join `#flowtask-admin`
- **Promoted to manager** → auto-join `#flowtask-managers`
- **Demoted from admin** → removed from `#flowtask-admin`
- **Demoted from manager** → removed from `#flowtask-managers`
...
Files are uploaded to the Chat App server via multipart/form-data and stored on disk. Metadata is persisted in the `ChatUpload` MongoDB model.

```
Client → POST /api/chat/channels/:channelId/upload (multipart/form-data)
       → Multer middleware processes files
       → ChatUpload documents created in MongoDB
       → File URLs returned as /api/chat/uploads/{uuid-filename}
       → Client sends message with attachments[] referencing upload URLs
```
...
| Parameter | Value |
|---|---|
| Max file size | 10 MB (`MAX_FILE_SIZE` env var) |
| Max files per request | 10 |
| Storage | Disk (`UPLOAD_DIR`, default: `./uploads`) |
| Filename strategy | UUID v4 + original extension |
| Static serving | `/api/chat/uploads` with 7-day cache |

### 10.3 Allowed MIME Types
...
| Code/Data | `application/json`, `application/xml`, `text/xml`, `application/x-yaml` |

### 10.4 Attachment Schema (Message Model)
...
```javascript
{
  ...
  url: String,            // Serving URL (/api/chat/uploads/...)
  thumbnailUrl: String,   // Optional thumbnail (null for non-images)
  source: 'upload' | 'flowtask',  // Origin of the file
  flowTaskAttachmentId: String     // FlowTask reference (if from FlowTask)
}
```

---

## 11. Chat Application API Reference

### 11.1 Authentication

| Method | Path | Description |
|---|---|---|
| `POST` | `/api/chat/auth/sync` | Sync user from FlowTask JWT |
| `GET` | `/api/chat/auth/me` | Get current chat user profile |
| `PUT` | `/api/chat/auth/preferences` | Update notification/theme preferences |
...
| `GET` | `/api/chat/channels/slug/:slug` | Get channel by slug |
...
| Method | Path | Description |
|---|---|---|
| `GET` | `/api/chat/channels/:channelId/messages` | List messages (paginated) |
| `POST` | `/api/chat/channels/:channelId/messages` | Send message |
| `POST` | `/api/chat/channels/:channelId/upload` | Upload files |
| `GET` | `/api/chat/channels/:channelId/pins` | List pinned messages |
...
| `GET` | `/api/chat/threads/:id/replies` | List replies (paginated) |
| `GET` | `/api/chat/threads/task/:taskId` | Get thread by FlowTask task ID |
| `GET` | `/api/chat/channels/:channelId/threads` | List channel threads |
...
| `POST` | `/api/chat/bot/command` | Execute bot command |
| `POST` | `/api/chat/webhooks/flowtask` | FlowTask webhook receiver |
...
### 12.2 Glossary: FlowTask ↔ Chat Mapping
...
### 12.3 Socket.IO Event Reference
...
### 12.4 Notification Type Reference
...
| 2.0.0 | 2025-01-15 | Chat App Team | Added implementation status (§9), file upload system (§10), Chat App API reference (§11). Updated FlowTask API URL. Documented all 15 webhook handlers, bot notification routing, security enhancements, and manager auto-join logic. |
