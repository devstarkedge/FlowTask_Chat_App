...
import { X, Users, Hash, Lock, Settings, UserPlus, LogOut, ChevronDown, Info, Globe } from 'lucide-react'
...
export default function ChannelInfoPanel({ channel, onOpenProfile }) {
  const { membersByChannel, isMembersLoading, setShowInfoPanel, removeMember, leaveChannel } = useChannelStore()
  ...
  const handleRemoveMember = async (memberId) => await removeMember(channel._id, memberId)
  const handleLeave = async () => { await leaveChannel(channel._id); setShowInfoPanel(false) }

  return (
    <>
      {/* ── Root panel── */}
      <div
        className="profile-panel cip-root"
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <h2 style={{
                  fontSize: 16, fontWeight: 800, color: 'var(--text-white)',
            ...
            <div className="cip-stats">
              ...
              {isPrivate
                ? <span className="cip-stat-pill private"><Lock size={10} /> Private</span>
                : <span className="cip-stat-pill public"><Globe size={10} /> Public</span>
              }
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '14px 14px 20px' }}>
          ...
          <div>
              ...
              <div style={{ marginBottom: 4 }}>
                ...
                <div className="panel-list">
                  {onlineMembers.map((member) => (
                    <div key={member._id || member.flowTaskUserId} className="panel-item" style={{ padding: '6px 8px', borderRadius: 'var(--radius-md)' }}>
                      <MemberItem
                        member={member}
            ...
            {offlineMembers.length > 0 && (
              <div>
                <div className="cip-members-section-label offline-label" style={{ marginTop: onlineMembers.length > 0 ? 12 : 0 }}>
                  <span className="dot" />
                  Offline — {offlineMembers.length}
                </div>
                <div className="panel-list">
                  {offlineMembers.map((member) => (
                    <div key={member._id || member.flowTaskUserId} className="panel-item" style={{ padding: '6px 8px', borderRadius: 'var(--radius-md)', opacity: 0.75 }}>
                      <MemberItem
                        member={member}
                        onOpenProfile={onOpenProfile}
                        canRemove={isAdmin && member._id !== user?._id && !isDM && !isSystem}
            ...
            {members.length === 0 && !isMembersLoading && (
              <div className="cip-empty">
                ...
                <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-secondary)', margin: 0 }}>No members yet</p>
                <p style={{ fontSize: 12, color: 'var(--text-muted)', margin: 0 }}>Add someone to get started</p>
                {isAdmin && !isDM && (
                  <button
                    onClick={() => setShowAddMember(true)}
        ...
        {confirmLeave && (
          <div className="cip-confirm-overlay">
            <div className="cip-confirm-card">
              ...
              <p style={{ fontSize: 15, fontWeight: 800, color: 'var(--text-white)', marginBottom: 6 }}>
                Leave channel?
              </p>
              <p style={{ fontSize: 13, color: 'var(--text-muted)', lineHeight: 1.55, marginBottom: 18 }}>
                You'll need to be invited back to rejoin <strong style={{ color: 'var(--text-secondary)' }}>#{channel.name}</strong>.
              </p>
