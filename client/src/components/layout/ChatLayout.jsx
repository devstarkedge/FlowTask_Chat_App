...
import { getActivityPath, getFilesPath, getChannelPath, getDMPath } from '../../utils/chatRoutes'
import { getNotificationText, normalizeNotification } from '../../utils/notificationFormat'
import {
  Activity, ArrowLeft, ArrowRight, Bell, ChevronRight,
  CircleHelp, Download, File, FileText, Info, Search,
  MessageSquare, Zap, FolderOpen, Eye, ExternalLink,
  Volume2, Image as ImageIcon, Film,
} from 'lucide-react'
...
export default function ChatLayout() {
  ...
  const {
    fetchChannels, activeChannelId, channels,
    showInfoPanel, setActiveChannel, unreads,
  } = useChannelStore()
  ...
  const handleDownload = async (file) => {
    ...
    try {
      addDownload({ name: fileName, url: downloadUrl, size: file.fileSize || file.size, type: file.mimeType || file.type })
  ...
  const handleOpenSearchResult = useCallback((item) => {
    ...
    switch (item.type) {
      case 'user': useProfileStore.getState().openProfile({ _id: item.id, name: item.name, email: item.email, avatar: item.avatar, role: item.role, onlineStatus: item.status, customStatus: item.customStatus, flowTaskUserId: item.flowTaskUserId }); break
      case 'message': navigate(item.channelType === 'dm' ? getDMPath(workspaceId, item.channelId, item.id) : getChannelPath(workspaceId, item.channelId, item.id)); break
      case 'channel': navigate(getChannelPath(workspaceId, item.id)); break
      case 'dm': navigate(getDMPath(workspaceId, item.id)); break
      case 'file': navigate(getFilesPath(workspaceId, item.referenceId)); break
      case 'link': navigate(item.channelType === 'dm' ? getDMPath(workspaceId, item.channelId, item.messageId) : getChannelPath(workspaceId, item.channelId, item.messageId)); break
      case 'page':
  ...
  const persistWidth = useCallback((w) => { try { localStorage.setItem(SIDEBAR_STORAGE_KEY, String(w)) } catch { } }, [])

  const handleResizeStart = useCallback((e) => {
    e.preventDefault()
    isResizingRef.current = true; setIsResizing(true)
    const startX = e.clientX, startW = sidebarCollapsed ? SIDEBAR_MIN : sidebarWidth
    document.body.style.cursor = 'col-resize'; document.body.style.userSelect = 'none'
    const onMove = (ev) => { const delta = ev.clientX - startX; setSidebarWidth(Math.min(SIDEBAR_MAX, Math.max(SIDEBAR_MIN, startW + delta))) }
    const onUp = () => {
      isResizingRef.current = false; setIsResizing(false)
  ...
  const handleResizeDoubleClick = useCallback(() => {
    if (sidebarCollapsed) { const r = widthBeforeCollapseRef.current; setSidebarWidth(r); persistWidth(r) }
    else { widthBeforeCollapseRef.current = sidebarWidth; setSidebarWidth(SIDEBAR_COLLAPSED); persistWidth(SIDEBAR_COLLAPSED) }
  }, [sidebarCollapsed, sidebarWidth, persistWidth])

  const globalSearchRef = useRef(null)

  const shortcutHandlers = useMemo(() => ({
    toggleSearch: () => { globalSearchRef.current?.focus(); setShowPins(false); setShowAllThreads(false); setShowNotifications(false) },
  ...
  const resetIdleTimer = useCallback(() => {
    if (isIdleRef.current) { isIdleRef.current = false; emitPresenceUpdate('online') }
    clearTimeout(idleTimerRef.current)
    idleTimerRef.current = setTimeout(() => { isIdleRef.current = true; emitPresenceUpdate('away') }, 5 * 60 * 1000)
  }, [])
  useEffect(() => {
    const events = ['mousemove', 'keydown', 'click', 'scroll', 'touchstart']
    events.forEach((e) => window.addEventListener(e, resetIdleTimer, { passive: true }))
    resetIdleTimer()
    return () => { events.forEach((e) => window.removeEventListener(e, resetIdleTimer)); clearTimeout(idleTimerRef.current) }
  }, [resetIdleTimer])

  usePushSubscription({ enabled: !!user })

  useEffect(() => {
    const handleFocus = () => { const cId = useChannelStore.getState().activeChannelId; if (cId) getSocket()?.emit('window:focus', { channelId: cId }) }
    const handleBlur = () => { getSocket()?.emit('window:blur', {}) }
  ...
  useEffect(() => { if (activeWorkspaceId) fetchChannels(activeWorkspaceId) }, [fetchChannels, activeWorkspaceId])
  ...
  const routeConversationId = channelMessageRoute?.params?.channelId || channelRoute?.params?.channelId || dmsMessageRoute?.params?.dmId || dmsRoute?.params?.dmId || null
  ...
  const selectedNotification = useMemo(() => notifications.find((n) => n._id === activityNotificationId) || null, [notifications, activityNotificationId])
  const selectedFile = useMemo(() => filesForModule.find((f) => f.referenceId === filesSelectedFileId) || null, [filesForModule, filesSelectedFileId])

  useEffect(() => {
    if (!workspaceId) return
    if (legacyDmMessageRoute?.params?.dmId) { navigate(getDMPath(workspaceId, legacyDmMessageRoute.params.dmId, legacyDmMessageRoute.params.messageId), { replace: true }); return }
    if (legacyDmRoute?.params?.dmId) { navigate(getDMPath(workspaceId, legacyDmRoute.params.dmId), { replace: true }) }
  }, [workspaceId, legacyDmMessageRoute, legacyDmRoute, navigate])

  useEffect(() => { if (!routeConversationId || routeConversationId === activeChannelId) return; setActiveChannel(routeConversationId) }, [routeConversationId, activeChannelId, setActiveChannel])
  useEffect(() => { if (activeChannelId && document.hasFocus()) getSocket()?.emit('window:focus', { channelId: activeChannelId }) }, [activeChannelId])

  const lastRouteMessageRef = useRef(null)
  useEffect(() => {
    if (!routeMessageId || lastRouteMessageRef.current === routeMessageId) return
    lastRouteMessageRef.current = routeMessageId
    useChatStore.getState().setHighlightMessageId(routeMessageId)
    const t = setTimeout(() => useChatStore.getState().setHighlightMessageId(null), 3500)
    ...
    const syncDeepLinkMessage = async () => {
      try {
        ...
        if (preferredChannelId) {
          try {
            ...
            if (!cancelled && aroundItems.length > 0) {
              ...
              if (preferredChannelId !== activeChannelId) setActiveChannel(preferredChannelId)
              return
            }
          } catch { }
        }
        ...
        if (routeChannelId && routeChannelId !== messageChannelId) {
          ...
          navigate(isDM ? getDMPath(workspaceId, messageChannelId, routeMessageId) : getChannelPath(workspaceId, messageChannelId, routeMessageId), { replace: true })
          return
        }
        useChatStore.getState().addMessage({ ...message, channelId: messageChannelId })
        if (messageChannelId !== activeChannelId) setActiveChannel(messageChannelId)
      } catch { }
    }
    syncDeepLinkMessage()
  ...
  useEffect(() => {
    ...
    const nextDM = [...dmChannels].sort((a, b) => {
      const aU = unreads[a._id] || 0, bU = unreads[b._id] || 0
      if (aU > 0 && bU === 0) return -1; if (aU === 0 && bU > 0) return 1
      return (b.lastMessageAt ? new Date(b.lastMessageAt).getTime() : 0) - (a.lastMessageAt ? new Date(a.lastMessageAt).getTime() : 0)
    })[0]
    if (nextDM?._id) navigate(getDMPath(workspaceId, nextDM._id), { replace: true })
  }, [workspaceId, dmsHomeRoute, channels, navigate, unreads])

  useEffect(() => { setShowMobileSidebar(false) }, [activeChannelId])

  const openThread = (thread) => {
    const channelId = typeof thread.channelId === 'object' ? thread.channelId._id : thread.channelId
    if (channelId && channelId !== activeChannelId) useChannelStore.getState().setActiveChannel(channelId)
    const rootMessageId = typeof thread.rootMessageId === 'object' ? thread.rootMessageId._id : thread.rootMessageId
    if (rootMessageId) { useChatStore.getState().setHighlightMessageId(rootMessageId); setTimeout(() => useChatStore.getState().setHighlightMessageId(null), 3000) }
    openThreadAction(thread); useProfileStore.getState().closeProfile(); setShowAllThreads(false)
  }

  const openProfile = (u) => { useProfileStore.getState().openProfile(u); closeThread(); useChannelStore.getState().setShowInfoPanel(false) }
  const openFilePreview = (file, allFiles = []) => { setPreviewFile(file); setPreviewFiles(allFiles.length > 0 ? allFiles : [file]) }
  ...
  const handleSelectActivityNotification = useCallback((notification) => {
    ...
    if (channelId) {
      ...
      const channelType = data.conversationType || data.channelId?.type || channels.find((c) => c._id === channelId)?.type
      navigate(channelType === 'dm' ? getDMPath(workspaceId, channelId, messageId) : getChannelPath(workspaceId, channelId, messageId))
    } else { navigate(getActivityPath(workspaceId, data._id)) }
    if (data.sourceType === 'message' && (data.messageId || data.sourceId)) {
      const messageId = asId(data.messageId || data.sourceId)
      if (messageId) { useChatStore.getState().setHighlightMessageId(messageId); setTimeout(() => useChatStore.getState().setHighlightMessageId(null), 3500) }
    }
  ...
  const handleSelectFileForModule = useCallback((file) => { if (!workspaceId || !file?.referenceId) return; navigate(getFilesPath(workspaceId, file.referenceId)) }, [workspaceId, navigate])

  const handleOpenFileInChat = useCallback(() => {
    if (!workspaceId || !selectedFile) return
    const channelId = asId(selectedFile.channelId); if (!channelId) return
  ...
  return (
    <div className="h-full flex" style={{ background: 'var(--bg-primary)' }}>
      <div className="hide-on-mobile">
        <ErrorBoundary name="WorkspaceSidebar" compact><WorkspaceSidebar /></ErrorBoundary>
      </div>
      ...
      <div className="hide-on-mobile relative" style={{ width: sidebarWidth, minWidth: sidebarWidth, transition: isResizing ? 'none' : 'width 200ms ease, min-width 200ms ease' }}>
        <ErrorBoundary name="ContextSidebar" compact>
          {isActivityRoute ? (
            <ActivityContextSidebar selectedNotificationId={activityNotificationId} onSelectNotification={handleSelectActivityNotification} onAutoSelect={handleAutoSelectActivityNotification} />
          ) : isFilesRoute ? (
            <FilesContextSidebar selectedFileId={filesSelectedFileId} onSelectFile={handleSelectFileForModule} onFilesChanged={setFilesForModule} />
          ) : (
            <NavigationSidebar mode={isDMRoute ? 'dms' : 'home'}
              onToggleAllThreads={() => { setShowAllThreads((s) => !s); setShowSearch(false); setShowPins(false); setShowNotifications(false); setShowSaved(false); useProfileStore.getState().closeProfile(); closeThread() }}
              onToggleNotifications={() => { setShowNotifications((s) => !s); setShowAllThreads(false); setShowSearch(false); setShowPins(false); setShowSaved(false); useProfileStore.getState().closeProfile(); closeThread() }}
              onToggleSaved={() => { setShowSaved((s) => !s); setShowAllThreads(false); setShowSearch(false); setShowPins(false); setShowNotifications(false); useProfileStore.getState().closeProfile(); closeThread() }}
            />
          )}
        </ErrorBoundary>
        <div className="sidebar-resize-handle" onMouseDown={handleResizeStart} onDoubleClick={handleResizeDoubleClick} title="Drag to resize, double-click to collapse" />
      </div>

      {showMobileSidebar && (
        <>
          <div className="sidebar-overlay active" onClick={() => setShowMobileSidebar(false)} />
          <div className="sidebar-mobile">
            <ErrorBoundary name="ContextSidebar" compact>
              {isActivityRoute ? (
                <ActivityContextSidebar selectedNotificationId={activityNotificationId} onSelectNotification={(n) => { handleSelectActivityNotification(n); setShowMobileSidebar(false) }} onAutoSelect={handleAutoSelectActivityNotification} />
              ) : isFilesRoute ? (
                <FilesContextSidebar selectedFileId={filesSelectedFileId} onSelectFile={(f) => { handleSelectFileForModule(f); setShowMobileSidebar(false) }} onFilesChanged={setFilesForModule} />
              ) : (
                <NavigationSidebar mode={isDMRoute ? 'dms' : 'home'} onClose={() => setShowMobileSidebar(false)} onToggleNotifications={() => { setShowNotifications((s) => !s); setShowMobileSidebar(false); setShowAllThreads(false); setShowSearch(false); setShowPins(false); useProfileStore.getState().closeProfile(); closeThread() }} />
              )}
            </ErrorBoundary>
          </div>
        </>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <GlobalTopBar
          ...
          onNotifications={() => { setShowNotifications((s) => !s); setShowPins(false) }}
        ...
        <div className="flex-1 flex min-w-0">
          <ErrorBoundary name="Content">
            {(() => {
              if (isActivityRoute) return <ActivityMainPane selectedNotification={selectedNotification} selectedChannelId={resolveNotificationChannelId(selectedNotification)} onOpenMobileSidebar={() => setShowMobileSidebar(true)} />
              if (isFilesRoute) return <FilesMainPane selectedFile={selectedFile} files={filesForModule} onPreview={(f) => { setPreviewFile(f); setPreviewFiles(filesForModule) }} onDownload={handleDownload} onOpenInChat={handleOpenFileInChat} onOpenMobileSidebar={() => setShowMobileSidebar(true)} />
              const matchedEntry = Object.entries(PAGE_ROUTES).find(([key]) => matchPath(`/workspace/:workspaceId/${key}`, location.pathname))
              const PageComponent = matchedEntry?.[1] ?? null
              if (PageComponent) return (
                <Suspense fallback={<div className="flex-1 flex items-center justify-center" style={{ background: 'var(--bg-primary)' }}><div className="cl-spinner" /></div>}>
                  <PageComponent />
                </Suspense>
              )
              return activeChannelId ? (
                <ChatPanel channelId={activeChannelId} onOpenThread={openThread} onToggleSearch={() => { setShowSearch((s) => !s); setShowPins(false) }} onTogglePins={() => { setShowPins((s) => !s); setShowSearch(false) }} onOpenProfile={openProfile} onOpenFilePreview={openFilePreview} onOpenMobileSidebar={() => setShowMobileSidebar(true)} onSaveMessage={handleSaveMessage} />
              ) : (
      ...
      {activeThread && <ErrorBoundary name="Thread Panel"><ThreadPanel thread={activeThread} onClose={closeThread} /></ErrorBoundary>}
      {showInfoPanel && activeChannel && !activeThread && !showSearch && !showPins && !profileUser && <ErrorBoundary name="Channel Info" compact><ChannelInfoPanel channel={activeChannel} onOpenProfile={openProfile} /></ErrorBoundary>}
      {showPins && activeChannelId && !activeThread && <ErrorBoundary name="Pinned Messages" compact><PinnedMessagesPanel channelId={activeChannelId} onClose={() => setShowPins(false)} /></ErrorBoundary>}
      {showAllThreads && !activeThread && <ErrorBoundary name="All Threads" compact><AllThreadsPanel onClose={() => setShowAllThreads(false)} onOpenThread={openThread} /></ErrorBoundary>}
      {profileUser && <ErrorBoundary name="Profile" compact><ProfileSidePanel user={profileUser} onClose={() => useProfileStore.getState().closeProfile()} /></ErrorBoundary>}
      {showNotifications && <ErrorBoundary name="Notifications" compact><NotificationPanel onClose={() => setShowNotifications(false)} onSelectNotification={handleSelectActivityNotification} /></ErrorBoundary>}
      {showSaved && <ErrorBoundary name="Saved Messages" compact><SavedMessagesPanel onClose={() => setShowSaved(false)} onJumpToMessage={(msg) => { if (msg.channelId !== activeChannelId) useChannelStore.getState().setActiveChannel(msg.channelId); setShowSaved(false) }} /></ErrorBoundary>}
      {showSearch && <ErrorBoundary name="Search" compact><SearchPanel channelId={activeChannelId} onClose={() => setShowSearch(false)} onJumpToMessage={(msg) => { if (msg.channelId !== activeChannelId) useChannelStore.getState().setActiveChannel(msg.channelId); setShowSearch(false) }} /></ErrorBoundary>}
      {previewFile && <FilePreviewModal file={previewFile} files={previewFiles} onClose={() => { setPreviewFile(null); setPreviewFiles([]) }} />}
      {showShortcuts && <KeyboardShortcutsModal onClose={() => setShowShortcuts(false)} />}
      {showTopPreferences && <PreferencesModal onClose={() => setShowTopPreferences(false)} />}
      {showTopSetStatus && <SetStatusModal onClose={() => setShowTopSetStatus(false)} />}
...
function GlobalTopBar({ user, workspaceId, searchRef, unreadCount, onBack, onForward, onOpenSearchResult, onNotifications, onHelp }) {
  const prevCountRef = useRef(unreadCount)
  const [bellShake, setBellShake] = useState(false)
  useEffect(() => {
    if (unreadCount > prevCountRef.current) { setBellShake(true); setTimeout(() => setBellShake(false), 600) }
  ...
  return (
    <header className="cl-topbar">
      <div className="cl-topbar__nav">
        <button className="cl-topbar__nav-btn" onClick={onBack} aria-label="Go back" title="Go back">
          <ArrowLeft size={15} />
        </button>
        <button className="cl-topbar__nav-btn" onClick={onForward} aria-label="Go forward" title="Go forward">
          <ArrowRight size={15} />
        </button>
      </div>

      <div className="cl-topbar__search-wrap">
        <GlobalSearch ref={searchRef} user={user} workspaceId={workspaceId} onOpenResult={onOpenSearchResult} />
      </div>
      ...
      <div className="cl-topbar__actions">
        ...
        >
          <Bell size={16} />
          {unreadCount > 0 && (
            <span className="cl-notif-badge">{unreadCount > 99 ? '99+' : unreadCount}</span>
          )}
        </button>
        <button className="cl-topbar__action-btn" onClick={onHelp} aria-label="Keyboard shortcuts" title="Keyboard shortcuts">
          <CircleHelp size={16} />
        </button>
      </div>
    </header>
  ...
  return (
    <div className="cl-welcome" style={{ background: 'var(--bg-primary)' }}>
      <div className="cl-welcome__card">
        <button className="cl-welcome__mobile-btn" onClick={onOpenMobileSidebar}>
          Open sidebar
        </button>
        ...
        <p className="cl-welcome__desc">
          Select a channel or DM from the sidebar to start collaborating with your team.
        </p>

        <div className="cl-welcome__pills">
          <span className="cl-welcome__pill">
            <span className="cl-welcome__pill-dot" style={{ background: 'var(--status-online, #22c55e)', animationDelay: '0ms' }} />
            Real-time messaging
          </span>
          <span className="cl-welcome__pill">
            <span className="cl-welcome__pill-dot" style={{ background: 'var(--accent-color, var(--accent-primary))', animationDelay: '300ms' }} />
            Project channels
          </span>
          <span className="cl-welcome__pill">
            <span className="cl-welcome__pill-dot" style={{ background: '#a78bfa', animationDelay: '600ms' }} />
            File sharing
          </span>
        </div>
      </div>
      ...
      {!selectedNotification && (
        <div className="cl-empty-pane">
          <div className="cl-empty-pane__inner">
            ...
            <div className="cl-empty-pane__icon-wrap" style={{ background: 'color-mix(in srgb, var(--accent-color, var(--accent-primary)) 10%, var(--bg-secondary))', borderColor: 'color-mix(in srgb, var(--accent-color, var(--accent-primary)) 20%, transparent)' }}>
            ...
            <p className="cl-empty-pane__sub">Choose a notification from the list to open its related conversation.</p>
          </div>
        </div>
      )}
      ...
      {selectedNotification && !selectedChannelId && (
        <div className="cl-empty-pane">
          <div className="cl-empty-pane__inner">
            ...
            <p className="cl-empty-pane__sub">This activity doesn't link to a specific channel or DM.</p>
          </div>
        </div>
      )}
      ...
      {!selectedFile ? (
        <div className="cl-empty-pane">
          <div className="cl-empty-pane__inner">
            ...
            <div className="cl-empty-pane__icon-wrap" style={{ background: 'color-mix(in srgb, #3b82f6 10%, var(--bg-secondary))', borderColor: 'color-mix(in srgb, #3b82f6 20%, transparent)' }}>
              <FolderOpen size={26} style={{ color: '#3b82f6', opacity: 0.8 }} />
            </div>
            <p className="cl-empty-pane__title">Select a file</p>
            <p className="cl-empty-pane__sub">Choose a file from the list to preview it or jump to its conversation.</p>
          </div>
        </div>
      ) : (
          ...
          <div className="cl-file-preview-shell">
            <div className="cl-file-preview-card">
              ...
              <div className="cl-file-preview-card__header">
                <div className="cl-file-preview-card__icon" style={{ background: `color-mix(in srgb, ${fileIconColor} 14%, transparent)`, border: `1px solid color-mix(in srgb, ${fileIconColor} 22%, transparent)` }}>
