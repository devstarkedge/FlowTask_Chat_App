...
const EMPTY_RESULTS_GLOBAL = {
  topMatches: [], users: [], messages: [], channels: [],
  dms: [], files: [], links: [], pages: [],
}
...
const GLOBAL_SECTION_KEYS = [
  'topMatches', 'users', 'messages', 'channels', 'dms', 'files', 'links', 'pages',
]
...
function historyKey(userId, workspaceId) {
  return `global_search_history_${userId || 'anon'}_${workspaceId || 'workspace'}`
}

function normalizeHistory(raw = {}) {
  const queries = Array.isArray(raw.queries)
    ? raw.queries.filter((q) => typeof q === 'string' && q.trim()).slice(0, MAX_RECENT_SEARCHES)
    : []
...
function readHistory(userId, workspaceId) {
  try {
    const parsed = JSON.parse(localStorage.getItem(historyKey(userId, workspaceId)) || 'null')
...
function writeHistory(userId, workspaceId, next) {
  localStorage.setItem(historyKey(userId, workspaceId), JSON.stringify(normalizeHistory(next)))
}
...
function getItemKey(item) {
  return [
    item?.type, item?.referenceId, item?.messageId,
    item?.channelId, item?.path, item?.id, item?.url, item?.label,
  ].filter(Boolean).join(':')
}
...
function toHistoryItem(item) {
  ...
  return {
    id: item.id, type: item.type, name: item.name, label: item.label, title: item.title,
    avatar: item.avatar, status: item.status, email: item.email, role: item.role,
    customStatus: item.customStatus, flowTaskUserId: item.flowTaskUserId,
    channelId: item.channelId, channelType: item.channelType, channelName: item.channelName,
    visibility: item.visibility, memberCount: item.memberCount, topic: item.topic,
    description: item.description, snippet: item.snippet, mimeType: item.mimeType,
    uploadedBy: item.uploadedBy, referenceId: item.referenceId, messageId: item.messageId,
    url: item.url, path: item.path, iconKey: item.iconKey, reasonLabel: 'Recent open',
  }
}
...
function saveRecentClick(userId, workspaceId, item) {
  ...
  return updateHistory(userId, workspaceId, (h) => {
    ...
    if (hi.type === 'user' && hi.id) {
      ...
      nextPeople[hi.id] = {
        ...cur, ...hi, count: (cur.count || 0) + 1,
        lastClickedAt: Date.now(), reasonLabel: 'Frequent teammate',
      }
    }
...
function sortByActivity(items) {
  return [...items].sort((a, b) => {
    const bt = new Date(b.lastMessageAt || b.updatedAt || b.createdAt || b.timestamp || 0).getTime()
    const at = new Date(a.lastMessageAt || a.updatedAt || a.createdAt || a.timestamp || 0).getTime()
    return bt - at
  })
}
...
function toConversationSuggestion(channel, reasonLabel) {
  ...
  return {
    id: channel._id, type: channel.type === 'dm' ? 'dm' : 'channel',
    name: channel.name || channel.slug || 'Conversation', slug: channel.slug,
    description: channel.lastMessagePreview || channel.description, topic: channel.topic,
    visibility: channel.visibility, memberCount: channel.memberCount, channelType: channel.type,
    lastMessageAt: channel.lastMessageAt, reasonLabel,
  }
}
...
function toDraftSuggestion(draft, channel) {
  ...
  return {
    id: channel._id, type: channel.type === 'dm' ? 'dm' : 'channel',
    name: channel.name || channel.slug || 'Conversation', slug: channel.slug,
    description: draft.text || 'Draft in progress', topic: channel.topic,
    visibility: channel.visibility, memberCount: channel.memberCount, channelType: channel.type,
    timestamp: draft.timestamp, reasonLabel: 'Draft conversation',
  }
}
...
function quickPages() {
  return [
    ...
    { id: 'files',         label: 'Files',         type: 'page', path: 'files',         iconKey: 'files'    },
  ]
}
...
function buildEmptyStateResults({ history, channels, drafts, workspaceId }) {
  ...
  const recentSearches = history.queries.map((q) => ({
    id: `recent:${q}`, type: 'recentSearch', label: q, reasonLabel: 'Recent search',
  }))

  const recentClicks = history.clicks.map((c) => ({ ...c, reasonLabel: c.reasonLabel || 'Recent open' }))
  ...
  const starredChannels = sortByActivity(channels.filter((c) => !c.isArchived && (c.isPinned || c.isStarred)))
    .map((c) => toConversationSuggestion(c, c.isPinned ? 'Pinned channel' : 'Starred channel'))

  const recentConversations = sortByActivity(channels.filter((c) => !c.isArchived && c.lastMessageAt))
  ...
  const quickLinks = quickPages().map((p) => ({ ...p, reasonLabel: 'Quick page' }))

  const suggestedResults = [
    ...recentClicks, ...frequentPeople, ...starredChannels,
    ...recentConversations, ...draftConversations, ...quickLinks,
  ]
    .filter(Boolean)
    .filter((item, i, arr) => arr.findIndex((c) => getItemKey(c) === getItemKey(item)) === i)
...
const UnifiedSearch = forwardRef(function UnifiedSearch(
  ...
  const upsertChannelMessages = useChatStore((state) => state.upsertChannelMessages)
  ...
  const [history, setHistory]             = useState(() => readHistory(user?._id, workspaceId))
  ...
  const scopeTargetLabel = isContextual ? getScopeTargetLabel(resolvedScope) : 'workspace'
  const panelTitle = isContextual
    ? `Find messages in ${resolvedScope?.name || resolvedScope?.slug || 'this conversation'}`
  ...
  }, [isContextual, hasQuery, channelResults, recentMessages, visibleGlobalResults, emptyStateResults])

  // ── Open / Close ─────────────────────────────────────────────────────────

  const openSearch = useCallback((options = {}) => {
    const nextMode = options.mode === 'dm'
      ? 'dm'
      : options.mode === 'channel'
        ? 'channel'
        : 'global'
    const nextChannelId = nextMode === 'global' ? null : options.channelId || null
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      setLoading(true)
      try {
        const { data } = await searchAPI.search({ q: trimmed, signal: controller.signal })
  ...
  useEffect(() => {
    if (!isContextual || !effectiveOpen || !channelId || !workspaceId) return undefined
    ...
    const load = async () => {
      if (!trimmed) {
        ...
        if (cachedRecent) { applyState(cachedRecent); return }

        setLoading(true)
        try {
          const { data } = await messageAPI.list(channelId, { limit: RECENT_LIMIT })
          const items = data?.data?.items || []
          const normalized = normalizeRecentMessages(items, resolvedScope, RECENT_LIMIT)
      ...
      if (cachedSearch) { applyState(cachedSearch); return }

      setLoading(true)
      try {
        const { data } = await searchAPI.search({
          q: trimmed, scope: channelId, limit: RECENT_LIMIT, signal: controller.signal,
        })
        const normalized = normalizeSearchMessages(data?.data?.messages || [], resolvedScope)
  ...
  }, [isContextual, effectiveOpen, channelId, workspaceId, trimmed, recentMessages, resolvedScope, upsertChannelMessages])

  // ── Handlers ────────────────────────────────────────────────────────────

  const handleQueryChange = useCallback((e) => {
    const value = e.target.value
    if (isContextual) setQueryByChannel((prev) => ({ ...prev, [channelKey]: value }))
    else setQuery(value)
  ...
  const handleRemoveScope = useCallback(() => {
    ...
    setSearchState((prev) => ({ ...prev, mode: 'global', channelId: null, isOpen: true }))
    setHistory(readHistory(user?._id, workspaceId))
    requestAnimationFrame(() => inputRef.current?.focus())
  ...
  const handleKeyDown = useCallback((e) => {
    ...
    if (e.key === 'Tab' && isContextual && hasQuery && contextualActionCount > 0) {
      e.preventDefault()
    ...
    if (e.key === 'ArrowDown') { e.preventDefault(); setActiveIndex((i) => Math.min(i + 1, Math.max(rows.length - 1, 0))); return }
    if (e.key === 'ArrowUp')   { e.preventDefault(); setActiveIndex((i) => Math.max(i - 1, 0)); return }
    if (e.key === 'Home')      { e.preventDefault(); setActiveIndex(0); return }
    if (e.key === 'End')       { e.preventDefault(); setActiveIndex(Math.max(rows.length - 1, 0)); return }

    if (e.key === 'Enter') {
      e.preventDefault()
      if (isContextual) {
        if (hasQuery && activeIndex < contextualActionCount) { handleShowResultsPage(); return }
        const resultIdx = hasQuery ? activeIndex - contextualActionCount : activeIndex
        const item = hasQuery ? channelResults[resultIdx] : recentMessages[resultIdx]
  ...
  return (
    <>
      {effectiveOpen && <div className="global-search__backdrop" aria-hidden="true" />}

      <div
        ref={containerRef}
        className={`global-search${effectiveOpen ? ' is-open' : ''}${isContextual ? ' is-contextual' : ''}`}
      >
        <div
