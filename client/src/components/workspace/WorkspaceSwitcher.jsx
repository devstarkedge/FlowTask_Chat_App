...
const UnreadBadge = memo(function UnreadBadge({ count }) {
  ...
  return (
    <span
      className="badge badge-red wss-badge-pop"
  ...
  return (
    ...
    >
      ...
      <div className="wss-ws-row__info">
        <p
          className={`wss-ws-row__name${isActive ? " wss-ws-row__name--active" : ""}`}
        >
          {workspace.name}
        </p>
  ...
  return (
    <div className="wss-root" ref={rootRef}>
      ...
      >
        ...
        <span
          className={`wss-chevron${isOpen ? " wss-chevron--open" : ""}`}
          aria-hidden="true"
        >
      ...
      {isOpen && (
        ...
        >
          ...
          {/* ── Action rows ── */}
          {actions.map((a) => {
            const { key, handler, ...rest } = a;

            return <ActionRow key={key} {...rest} onClick={handler} />;
          })}
        </div>
      )}
    </div>
