...
export default function useDraftAutoSave(conversationId, threadId, editorRef) {
  ...
  const restoreDraft = useCallback(async () => {
    ...
    // Clear editor immediately so stale content never leaks during async restore
    ed.clear()
    lastContentRef.current = ''

    // Ensure Zustand persist has rehydrated drafts from localStorage
    ...
    // Try server if no local draft
    if (!draft && activeWorkspaceId) {
      try {
        const { data } = await draftAPI.get(conversationId, threadId)
        // Bail if channel changed while we were awaiting the API call
        if (gen !== restoreGenRef.current) return false
        if (data?.data?.draft) {
    ...
    // Final staleness check — channel may have changed during the await above
    if (gen !== restoreGenRef.current) return false
