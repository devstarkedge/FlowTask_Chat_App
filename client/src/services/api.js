...
export const messageAPI = {
  ...
  around: (channelId, messageId, params) =>
  ...
  edit: (id, content) => {
    ...
    // so rich formatting is preserved server-side.
    if (typeof content === "string")
      return api.put(`/messages/${id}`, { content });
    if (content && typeof content === "object") {
      const payloadContent = content.htmlContent ?? content.content ?? "";
      return api.put(`/messages/${id}`, { content: payloadContent });
    }
...
export const fileAPI = {
  ...
  listByChannel: (channelId, params) =>
    api.get(`/channels/${channelId}/files`, { params }),
  deleteFromChannel: (channelId, fileId) =>
    api.delete(`/channels/${channelId}/files/${fileId}`),
};
...
// ─── Bot ─────────────────────────────────────────────────────────────────
export const botAPI = {
  command: (command, channelId) =>
    api.post("/bot/command", { command, channelId }),
};
...
export const userAPI = {
  ...
  getDMContacts: (search) =>
    api.get("/users/dm-contacts", { params: { search } }),
};
...
export const notificationAPI = {
  ...
  updateChannelPreference: (channelId, pref) =>
    api.put(`/notifications/preferences/channel/${channelId}`, pref),
  removeChannelPreference: (channelId, section) =>
    api.delete(`/notifications/preferences/channel/${channelId}`, {
      data: { section },
    }),
  pauseNotifications: (data) =>
    api.put("/notifications/preferences/pause", data),
  resumeNotifications: () => api.post("/notifications/preferences/resume"),
  updateKeywords: (keywords) =>
    api.put("/notifications/preferences/keywords", { keywords }),
  updateVIPUsers: (vipUsers) =>
    api.put("/notifications/preferences/vip", { vipUsers }),
  // Push management
  dismissPush: (notificationId) =>
    api.post("/push/dismiss", { notificationId }),
...
export const savedMessageAPI = {
  list: (status = null) =>
    api.get("/messages/saved", { params: status ? { status } : {} }),
...
export const scheduledMessageAPI = {
  ...
  create: (channelId, data) =>
    api.post(`/channels/${channelId}/scheduled-messages`, data),
  cancel: (id) => api.delete(`/messages/scheduled/${id}`),
  reschedule: (id, scheduledAt) =>
    api.patch(`/messages/reschedule/${id}`, { scheduledAt }),
...
export const adminAPI = {
  ...
  archiveChannel: (channelId) =>
    api.post(`/admin/channels/${channelId}/archive`),
  unarchiveChannel: (channelId) =>
    api.post(`/admin/channels/${channelId}/unarchive`),
