  ...
  return Object.entries(drafts)
    ...
    .filter((draft) => {
      const hasAttachments =
        ...
        (draft.fileReferences?.length || 0) > 0;

      return (
        draft.channelId &&
        !isDraftExpired(draft, now) &&
        (!isContentEmpty(draft.html, draft.text) || hasAttachments)
      );
    })
  ...
  persist(
    (set, get) => ({
      ...
      setDraft: (
        ...
        const attachments = normalizeArray(metadata.attachments);
        const fileReferences = normalizeArray(metadata.fileReferences);

        if (
          isContentEmpty(trimmedHtml, trimmed) &&
