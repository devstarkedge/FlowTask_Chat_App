...
export const sendMessage = asyncHandler(async (req, res) => {
  const { content, htmlContent, contentType, attachments, fileReferences, flowTaskRef, threadId, tempId, mentions } = req.body;
...
export const getWorkspaceFiles = asyncHandler(async (req, res) => {
  ...
  const decodeCursor = (value) => {
    ...
    try {
      ...
      if (Number.isNaN(createdAt.getTime()) || !mongoose.Types.ObjectId.isValid(idRaw)) {
        return null;
      }
      return {
        createdAt,
  ...
  if (q) {
    fileMatch.originalName = { $regex: q.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), $options: 'i' };
  }

  const matchesKind = (mimeType = '') => {
    if (kind === 'image') return mimeType.startsWith('image/');
    if (kind === 'video') return mimeType.startsWith('video/');
    if (kind === 'file') return !mimeType.startsWith('image/') && !mimeType.startsWith('video/');
    return true;
  };
  ...
  while (collected.length < limit + 1 && !reachedEnd) {
    ...
    if (lastRef) {
      seek = { createdAt: new Date(lastRef.createdAt), id: new mongoose.Types.ObjectId(lastRef._id) };
    }
  ...
  const nextCursor = hasMore && visibleRefs.length > 0
